...
// ══════════════════════════════════════════
//  INIT — seed default admin if not exists
// ══════════════════════════════════════════
function init() {
  if (!DB.get('users')) {
...
//  SESSION
// ══════════════════════════════════════════
let session = { user: null, step: null };
...
async function doLogin() {
  ...
  if (!user.totpEnabled) {
    session.step = 'setup';
    generateTOTPSecret(user.username);
    show('screen2FASetup');
  } else {
    session.step = 'pass-ok';
    show('screen2FA');
...
function generateTOTPSecret(username) {
  ...
  document.getElementById('secretDisplay').textContent =
    tempSecret.secret.base32.match(/.{1,4}/g).join(' ');

  const qrEl = document.getElementById('qrContainer');
  qrEl.innerHTML = '';
  new QRCode(qrEl, {
    text: tempSecret.toString(),
    width: 180, height: 180,
    colorDark: '#000', colorLight: '#fff',
  });
}

function confirmSetup2FA() {
  const code = document.getElementById('setupCode').value.trim();
  if (!tempSecret) return;

  const delta = tempSecret.validate({ token: code, window: 1 });
  if (delta === null) {
    showAlert('alertSetup', 'Pogrešan kod. Proverite aplikaciju i pokušajte ponovo.');
    return;
  }

  const users = DB.get('users');
  const idx   = users.findIndex(u => u.username === session.user);
  users[idx].totpSecret  = tempSecret.secret.base32;
...
function verify2FA() {
  ...
  const users = DB.get('users');
  const user  = users.find(u => u.username === session.user);

  if (!user || !user.totpSecret) {
    showAlert('alert2FA', 'Greška sa 2FA. Kontaktirajte admina.');
    return;
  }
  ...
  if (delta === null) {
    showAlert('alert2FA', 'Pogrešan kod. Pokušajte ponovo.');
    document.getElementById('totpCode').value = '';
    return;
  }
...
function showTab(id) {
  ...
  const map = {
    tabClients:  'btnTabClients',
    tabLog:      'btnTabLog',
    tabCharts:   'btnTabCharts',
    tabSettings: 'btnTabSettings',
  };
...
function addClient() {
  ...
  if (clients.find(c => c.first === first && c.last === last)) {
    showAlert('alertClient', 'Klijent već postoji.');
    return;
  }
...
function renderClientList() {
  ...
  el.innerHTML = clients.map(c => {
    ...
    const last  = logs.filter(l => l.clientId === c.id)
      ...
      <div class="client-item" onclick="openClientModal(${c.id})">
        <div>
          ...
          <div class="client-meta">
            ${count} unos(a) ${last ? '· Poslednji: ' + last.date : '· Nema unosa'}
          </div>
        </div>
...
document.getElementById('logProgress').addEventListener('input', function () {
  document.getElementById('progPreview').style.width =
    Math.min(100, Math.max(0, this.value || 0)) + '%';
});
...
function renderCharts() {
  ...
  const buildChart = (id, color, data, unit) => {
  ...
  buildChart('chartProgress', '#4f8ef7', progress, '%');
  buildChart('chartWeight',   '#34d399', weights,  ' kg');
  buildChart('chartReps',     '#fbbf24', reps,      ' reps');
}
    ...
    ? cLogs.map(l => {
        ...
        return `
          <tr>
            ...
            <td>${l.reps}</td>
            <td>${l.weight ?? '—'}</td>
            <td style="color:${color}">${l.progress}%</td>
          </tr>`;
      }).join('')
...
async function changePassword() {
  ...
  if (users[idx].passHash !== oldHash) {
    showAlert('alertSettings', 'Pogrešna trenutna lozinka.');
    return;
  }
...
document.addEventListener('keydown', e => {
  ...
  if (active === 'screen2FA')      verify2FA();
  if (active === 'screen2FASetup') confirmSetup2FA();
});
